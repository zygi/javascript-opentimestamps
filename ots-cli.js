...
function info(argsFileOts, options) {
    const otsPromise = fsp.readFile(argsFileOts, null)
...
async function stamp(argsFiles, options) {
    ...
    if (options.digest) {
        ...
        argsFiles.forEach(argsFile => {
            filePromises.push(fsp.readFile(argsFile, null))
        })
    }
...
function verify(argsFileOts, options) {
    const files = []
    files.push(fsp.readFile(argsFileOts, null))
    if (options.digest) {
        // input is a digest
